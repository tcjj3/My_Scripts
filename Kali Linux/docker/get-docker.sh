...
do_install() {
	...
	case "$lsb_dist" in
		ubuntu|debian|raspbian|kali)
			...
			pre_reqs_=""
			if [ "$lsb_dist" = "debian" ] || [ "$lsb_dist" = "kali" ]; then
			...
			(
				...
				$sh_c 'apt-get update -qq >/dev/null' || $sh_c 'echo "continue..." >/dev/null'
				$sh_c "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq $pre_reqs >/dev/null" || $sh_c "DEBIAN_FRONTEND=noninteractive apt-get install -y -qq $pre_reqs_ >/dev/null"
